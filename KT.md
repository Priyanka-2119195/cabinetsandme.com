...
- **Hero visual redesign — full-bleed image + floating glass form (desktop only):** the hero image now spans the entire section (both columns) instead of sitting only behind `.hero-left`, with a slow ambient zoom (`@keyframes heroBgZoom`, 24s alternate — same technique already used on `lp-apartments-meta-01/index.html`, reused here rather than invented). `.hero-form` is no longer a solid white panel stretched to the column's full height; it's `align-self:center` with `margin:0 32px`, `max-height:calc(100% - 64px)`, `border-radius:20px`, a translucent `rgba(255,255,255,.86)` background, `backdrop-filter:blur(22px) saturate(160%)`, a soft `rgba(255,255,255,.65)` border and a diffuse shadow — a frosted-glass card floating over the photo instead of blocking it. Field inputs/selects got a matching `border-radius:6px` and a translucent `rgba(244,250,251,.9)` background for cohesion. Buttons were deliberately left sharp-edged (unchanged) to keep the site's flat, architectural button language — only the card and inputs got the softer treatment.
  - **Gotcha:** `.hero-form` uses `justify-content:flex-start`, not `center`. Centering the flex content *and* constraining height with `overflow-y:auto` is a classic trap — when content overflows a centered flex container, the scrollable area's default scroll position (0) lands in the middle of the (overflowing) centered content, not its true top, hiding the top edge (here: the form-eyebrow and part of the heading) with no way to tell anything was cut off. Fixed by anchoring content to the top instead; the card itself is still vertically centered as a whole via `align-self:center` on the grid.
  - **Mobile is unaffected on purpose:** the `≤1080px` media query overrides `.hero-form` back to a solid opaque white card (`backdrop-filter:none`), no `align-self`/`max-height` constraints — translucency + blur is a desktop-only refinement; mobile keeps the simpler, cheaper-to-render solid card with its existing gold `border-top:3px` accent.
