...
| Apartments (Meta 01) | `/lp-apartments-meta-01` | `lp-apartments-meta-01/index.html` | Lead gen for apartment interiors — Meta campaign, wizard-style hero form, functional mobile drawer nav (not part of the original 6-LP set, no entry existed for this page before 2026-07-24) | "Schedule A Design Consultation" | Yes |
...
- **Unique sections:** `#villas` section + `#next` + `#midform`
- **Performance + accessibility pass (2026-07-24) — PSI mobile score 46 → local Lighthouse 66; agentic-browsing 50 → 100; accessibility 88 → 96.** Applied the full nri-homeowners playbook (§20): `#hero .r{opacity:1;}` LCP fix, GTM/Meta-Pixel/Orbyo-Intel deferred off the previously-blocking `<head>` tags and staggered via `_loadOnIdle`, jQuery+Orbyo lead-submission loader moved off static `<script>` tags to `window.load` (with the matching `sub()` wait-for-orbyo hardening), the Google Fonts stylesheet moved from a blocking `<link rel="stylesheet">` to the preload+`media="print"` swap trick, `for`/`id` added to every form label (fixed `select-name`, the actual cause of this page's `agentic-browsing` failure), and a `<main>` landmark added around the hero-through-studio content (fixed `landmark-one-main`). Color-contrast left untouched — same "design decision, not a perf/a11y bug" call made on nri-homeowners. No local images on this page (fully Wix-hosted) so there was nothing to recompress.
...
- **Second performance pass (2026-07-23/24) — PSI mobile score went 64 → 79 (local Lighthouse verification) → 96 mobile / 95 desktop (live PSI, per client).** Where the previous pass (above) covered structural/loading changes (picture elements, lazy backgrounds, CSS splitting), this pass targeted what PSI's Lighthouse audit actually flagged as remaining, verified against a real local Lighthouse run (the PSI API itself was rate-limited/quota-blocked at the time, `npx lighthouse` against the live URL was used instead). Full reusable method in **§20 — apply this before repeating any of it on the other 6 LPs.**
  ...
  - **Third pass, same session (2026-07-24) — Total Blocking Time was swinging the PSI score between the mid-80s and mid-90s run to run.** Diagnosed as GTM, the Meta Pixel, and the Orbyo Intel widget all firing on the exact same `window.load` tick and piling onto the main thread together — how badly they overlapped depended on third-party network timing (Google's/Facebook's/Orbyo's server response times on that particular run), which is inherently noisy and mostly outside this codebase's control. Fixed by giving each one its own `requestIdleCallback` (with a `setTimeout` fallback for browsers without it, e.g. Safari) at staggered timeouts (800ms / 1600ms / 2400ms) via a shared `window._loadOnIdle(fn, timeout)` helper defined once in the GTM block. Verified with 3 repeated local Lighthouse runs before/after: TBT spread went from 270–763ms (score 80–93) to 297–537ms (score 86–92) — same ballpark average, roughly half the variance.
    ...
    - **Found and fixed a real pre-existing data-loss gap while auditing this:** `sub()`'s fallback was a flat `if(typeof orbyo!=='undefined'){...} else {_showSuccess()}` — if a visitor submitted the form before the deferred Orbyo script had finished loading, the code silently skipped the CRM submission entirely and just showed the success redirect. No error, no retry, lead just never arrives anywhere. `sub()` now `await`s a up-to-5-second poll (150ms interval) for `orbyo` to become defined before giving up. **Update 2026-07-24: this fix has now also been applied to `lp-villa-recent-possession` and `lp-apartments-meta-01`** (see their own entries) as part of deferring their jQuery/Orbyo loaders too — the other 4 LPs still load these synchronously at the bottom of `<body>` (§7) and don't have this race yet, but **if you defer the loader on any of them, you must bring this fix with it.**

---

### `lp-apartments-meta-01/index.html` — Apartments Meta Campaign 01

**Not part of the original 6-LP set — no entry existed for this page before 2026-07-24, and it's still missing from the internal LP index (`index.html`).** More developed than the base template in several ways:
- **Functional mobile drawer nav** (`#mobDrawer`) — unlike the other LPs' non-functional hamburger (Known Issues #1), this one actually opens/closes (`mobBtn`/`mobDrawer` JS, toggles `.open`, `aria-expanded`, `aria-hidden`, closes on link click or Escape).
- **Wizard-style hero form** with a distinct success-animation state (`wiz-success`, `_animDone`/`_submitDone` tracked separately in `sub()` so the CRM submit and the on-screen success animation both complete before redirecting) — more involved than the other LPs' immediate-redirect `_showSuccess()`.
- Local images exist in this folder's git history (4 old PNGs + an `images/` subfolder of jpg+webp pairs) **but were never actually referenced anywhere in this page's HTML** — confirmed by grep before deleting them on 2026-07-24 (see below). This page is fully Wix-hosted for its actual content images.
- **Performance + accessibility pass (2026-07-24) — PSI mobile score 54 → local Lighthouse 71; agentic-browsing 1/2 (50) → 100; accessibility 90 → 97.** Same treatment as `lp-villa-recent-possession` above (LCP opacity fix, deferred+staggered GTM/Pixel/Orbyo-Intel, deferred jQuery+Orbyo lead loader with the `sub()` wait fix, async Google Fonts loading, `<main>` landmark), plus two fixes specific to this page's more advanced markup:
  - **`aria-hidden-focus` / this page's actual `agentic-browsing` failure cause:** `#mobDrawer` correctly toggled `aria-hidden` when closed, but never removed its links/CTA button from the tab order — they stayed keyboard-focusable while `aria-hidden="true"`, an invalid accessibility-tree state (confirmed via Lighthouse's `agent-accessibility-tree` audit, which names AI-agent navigation specifically as what this breaks, not just screen readers). Fixed by toggling the `inert` attribute on `mobDrawer` in lockstep with `aria-hidden` (`mobDrawer.inert=!open` in the click handler, `mobDrawer.inert=true` in `closeMobDrawer()`, plus the `inert` attribute on the initial closed-state HTML) — `inert` has no default visual styling, so this is purely a focus/accessibility-tree fix, zero visual change.
  - **`heading-order`:** `<h3 class="hero-cta-h">` inside the hero form appeared before the page's first `<h2>`, skipping a level. Changed to `<h2>` — safe because `.hero-cta-h`'s CSS fully controls font/size/weight/colour, nothing relies on the `<h3>` tag's default styling.
- **Deleted 12 orphaned local image files (2026-07-24):** `Bathroom_Cabinets_9-16.png`, `Kitchen_Cabinets_2_1-1.png`, `Sideboards_2_1-1.png`, `Walkin_Closet_2_9-16.png` (old, dated April 2026) and the entire `images/` folder (`bathroom`/`kitchen`/`living`/`wardrobe`, each `.jpg`+`.webp`, dated July 2026) — ~7.5MB total, none referenced in `index.html` or `thank-you/index.html`. Recoverable from git history if ever needed.
- **Still has the same "Agentic Browsing" `color-contrast` and gold-on-white issue as every other LP** — left untouched, same design-decision call as nri-homeowners.

---

### Thank-You Pages (8 pages total — identical structure, different quote/WA message)
...
### Critical
1. **Hamburger menu non-functional** — The `.mob-btn` button has no JS event listener. Clicking it on mobile does nothing. Nav links remain inaccessible on mobile. **`lp-apartments-meta-01` is the exception** — it has a fully working mobile drawer (`#mobDrawer`), though its `aria-hidden`/focus handling had a bug fixed on 2026-07-24 (see its §4 entry and §20 item 8).
...
17. **Form labels use `.field label` not `<label for="...">` association** — Labels are visually above inputs but are `<label>` elements without `for` attributes, so screen reader association depends on DOM proximity. **Fixed on `nri-homeowners` (2026-07-23)** — every label now has a matching `for`/`id`; the phone-code select (no visible label of its own) got `aria-label="Country dialling code"` instead. **Also fixed on `lp-villa-recent-possession` (2026-07-24)** — this was the actual cause of that page's Lighthouse `agent-accessibility-tree`/`select-name` failure (§20 item 8). `lp-apartments-meta-01` already had `for`/`id` associations before this pass (it's more developed than the base template). Still an issue on the remaining 4 LPs.
...
Everything below was done to `nri-homeowners` across two sessions (2026-07-22/23, then 2026-07-23/24) and took it from a PSI mobile score of 64 to 96 mobile / 95 desktop. The same pass was then applied to `lp-villa-recent-possession` (PSI 46 → local Lighthouse 66) and `lp-apartments-meta-01` (PSI 54 → local Lighthouse 71) on 2026-07-24. Four LPs still haven't had this applied: `lp-villa-kitchen`, `lp-villa-wardrobes`, `lp-villa-renovation`, `lp-villa-under-construction`, `lp-apartment-owners`. This section is the reusable checklist — read the full narrative in the relevant §4 entries first if anything here is unclear, that's where the reasoning and the "why" live.

**Before touching anything: measure first.** Don't guess at what's slow — run a real Lighthouse pass (`npx lighthouse "<url>" --form-factor=mobile --screenEmulation.mobile --throttling-method=simulate --output=json`, omit `--only-categories` so you get accessibility/SEO/agentic-browsing too) against the live page, or use PSI directly if it isn't rate-limited, and read the actual `lcp-breakdown-insight`, `bootup-time`, `image-delivery-insight`, `cache-insight`, and `agent-accessibility-tree` audit details before deciding what to change. The specific fixes below happened to be what each LP needed; another LP's bottleneck could be different — on `lp-villa-recent-possession` and `lp-apartments-meta-01` the LCP fix alone barely moved the score until the font-loading fix (item 1a) was added too, because unlike `nri-homeowners` at that point, their Google Fonts request was still render-blocking.

**A note on Lighthouse's categories:** as of the version installed here (`npx lighthouse`, v13.4.1, 2026-07), there are now **five** categories, not four — `performance`, `accessibility`, `best-practices`, `seo`, and a newer **`agentic-browsing`** ("Agentic Browsing... checks ensure high-quality, browsable websites for AI agents... still under development and subject to change" per its own description). Only two of its audits are actually weighted into the score (`agent-accessibility-tree` and `cumulative-layout-shift`, 50% each — this is why a failing page reads as "1/2" or 50%); the `webmcp-*` and `llms-txt` audits are informative-only (weight 0) for now. If a client reports an "agentic ai" score, this is what they mean — see item 8 below.
...
### 1a. Check whether the Google Fonts stylesheet is still render-blocking
`nri-homeowners` already had its font loading fixed in its *first* pass (2026-07-22/23), so item 1 alone was enough there. `lp-villa-recent-possession` and `lp-apartments-meta-01` still had a plain synchronous `<link href="https://fonts.googleapis.com/css2?..." rel="stylesheet">` — Lighthouse's `render-blocking-insight` flagged it directly (~850ms wasted), and it was eating most of the LCP element-render-delay that the opacity fix alone didn't touch. Fix (same trick already used for `nri-homeowners/styles.css`):
```html
...
```
Check every LP's `<head>` for this before assuming the LCP fix alone will move the score — it won't, if the fonts request is still blocking.
...
The remaining 4 un-migrated LPs load `jquery-3.7.1.min.js` and `olApiV2.min.js` as static, synchronous `<script src>` tags at the bottom of `<body>` (§7) — by the time a real visitor finishes filling out the form, they're already loaded, so this isn't a problem *today*. But if you move them to a `window.load`-deferred pattern (for the performance win — this is ~90KB+ of blocking script at the very bottom of every page), you introduce a race: someone who submits very fast could hit `sub()` before `orbyo` is defined. Every un-migrated LP's `sub()` has the same `if(typeof orbyo!=='undefined'){...}else{_showSuccess()}` fallback (§7), and the `else` branch silently skips the actual CRM submission — the visitor still sees success and gets redirected, but the lead never arrives at Orbyo. `nri-homeowners`, `lp-villa-recent-possession`, and `lp-apartments-meta-01` all now have the fix: `sub()` `await`s up to 5 seconds (150ms polling interval) for `orbyo` to become defined before falling back (each has its own copy of the same wait — `sub()` isn't shared across files, §7). Port that same wait, not just the deferred script tags, to any of the remaining 4 LPs where you defer this loader — note `lp-apartments-meta-01`'s `sub()` has a more involved wizard-style success-animation flow than the others, read its actual code before copy-pasting the wait block in.

### 4. Recompress images with `cwebp -m 6 -af`, not just a lower `-q`
`cwebp` is installed at `C:\Program Files\libwebp\bin\cwebp`. The prior nri-homeowners webp conversions (done at `-q 92 -m 6`, §4) were still 50–70% larger than necessary — method `6` (best compression search) plus `-af` (auto-filter) at a lower quality (`-q 65` to `-q 72` worked well for these photos) produced files 2–3x smaller with no visible difference. **Always visually spot-check a few re-encodes at 100% zoom side-by-side before committing** (see the memory note on full-page visual verification) — don't trust file-size deltas alone. If any other LP has local images (most don't yet — see §16 #15), or if any are migrated to local per item 5 below, run them through this same recompression. **Check for orphaned local images first, though** — `lp-apartments-meta-01` had ~7.5MB of local PNG/JPG/WebP files sitting in its folder that were never referenced anywhere in its HTML (confirmed by grep before deleting); recompressing dead files is wasted effort, delete them instead (§4 entry has the exact list).
...
### 8. Fixing `agentic-browsing` and its underlying accessibility failures
The category is really just testing whether the accessibility tree is well-formed (see the note above on how it's scored). In practice this means running the ordinary `accessibility` category audits and fixing what's flagged — every fix below also improved the plain `accessibility` score, not just `agentic-browsing`. Ones seen so far, cheapest first:
- **`landmark-one-main` (missing `<main>`):** every LP's markup goes `</nav>` (and a mobile drawer `<nav>` if present) → `<section id="hero">` → ... → `</section>` → `<footer>`. Wrap everything from the first content `<section>` to the last one (right before `<footer>`) in `<main>...</main>`. Checked both migrated LPs for `body >` CSS combinators or JS selectors that assume no wrapper element exists first — there weren't any, but check per-LP before assuming it's safe.
- **`select-name` (a `<select>` with no accessible name):** same root cause as Known Issues #17 — `<label>Text</label>` with no `for`, next to `<select id="...">`. Add `for="the-id"` to every field's label, not just the one Lighthouse happened to flag (it was inconsistent about which single field it named, and the real fix is "every label needs `for`", not "add it to whichever one gets reported").
- **`aria-hidden-focus` (an `[aria-hidden="true"]` element contains focusable descendants):** seen on `lp-apartments-meta-01`'s mobile drawer, which correctly toggled `aria-hidden` but never removed its links from the tab order while closed. Fix: toggle the `inert` attribute in lockstep with `aria-hidden` (`mobDrawer.inert = !open` alongside the existing `aria-hidden` toggle, plus `inert` on the initial closed-state HTML). `inert` has no default visual styling in any current browser — this is a pure focus/accessibility-tree fix. If another LP's non-functional hamburger (Known Issues #1) is ever made functional, build the `aria-hidden`+`inert` pairing in from the start rather than retrofitting it.
- **`heading-order` (heading levels skip, e.g. h1 → h3):** check every LP's actual heading sequence with `grep -n "<h1\|<h2\|<h3\|<h4"` before assuming it's fine — `lp-apartments-meta-01` had an `<h3>` inside the hero form appearing before the page's first `<h2>`. Renumbering a heading tag is safe as long as the class controls the visual style (font-size/weight/colour), not the tag's browser default — verify that before changing the tag.
- **`color-contrast`:** left untouched everywhere (gold-on-white, Known Issues #19) — this is a design decision (the client's brand gold), not a bug, and changing a colour is out of scope for a perf/accessibility pass unless the client explicitly asks for it.
