...
- **Hero background image — went Wixstatic CDN, then back to local, on a deliberate speed/sharpness trade-off.** The local NRI photo assets (`kitchen_1-1.jpg` etc.) are all portrait or square smartphone-style shots (472x590 for the `_1-1.jpg` set); using one as a full-width `background-size:cover` hero forces a large upscale (roughly 4x at a 1920px-wide desktop viewport) and produced a visibly softer, more tightly-cropped result than the sharp Wixstatic landscape photo. First fix was to point `.hero-bg` at the same Wixstatic photo used elsewhere on the site as a kitchen hero (`e3b82d_b4d5ab7c97dd4207a98affd10b4ee7e0~mv2.jpg`). Client then asked to switch back to a local image (`kitchen_1-1`) because the external CDN request was adding noticeable load latency. `.hero-bg` is now `kitchen_1-1.jpg`/`.webp` (image-set fallback, same technique as `.villas-bg`), served same-origin and preloaded (see below) — genuinely faster, at the cost of a slightly soft/upscaled look that's largely masked by the gradient overlay. `og:image`/`twitter:image` and `thank-you/index.html`'s `.ty-img-bg` were updated to match. **If this tension resurfaces:** the real fix is a properly-shot landscape/16:9 photo at a decent native resolution (1600px+ wide) supplied specifically for the hero, not picking between the two existing trade-offs.
